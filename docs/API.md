...
`PreviewLayout` measures geometry after the resolved item rotation, scale and model translation, excluding transparent pixels using a convex outline across animation frames. It caches bounds by fixture revision, resolved transform and baked model identity. `CompartmentFit` uniformly reduces oversized models in all three dimensions. The fit is applied in fixture coordinates before item orientation. All three rack sizes center their 3D models; shelves align their bottoms with the profile's Y anchor, including displayed block items. Lower shelf compartments use their shorter physical height. Actual item rendering remains live.

## Appearance crafting

`AppearanceRecipe` is a component-preserving special crafting recipe, registered as `reset_appearance` and `copy_appearance`. `matches`, `assemble` and remainder calculation leave every input stack untouched. Reset removes only the materials component; copying starts from the target stack and changes only its materials component. Copy recipes return one original template as a crafting remainder.

These are batch recipes: the plan's target slot is consumed in full, and the output retains that target's count. `AppearanceResultSlotMixin` applies the batch amount to normal crafting-grid consumption. `AppearanceCrafterMixin` does the same for the vanilla Crafter. Custom recipe executors integrating these recipes must also consume the target's complete count, rather than the usual one item per ingredient slot. Template consumption remains one, offset by its unchanged crafting remainder. `AppearanceQuickCraftMixin` stops shift-click repetition when the next fixture result has different components, preventing a copied batch from immediately resetting its remaining template.
