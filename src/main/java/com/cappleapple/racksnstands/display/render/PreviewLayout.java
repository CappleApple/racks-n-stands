...
final class PreviewLayout {
    ...
    private static final WeakHashMap<net.minecraft.client.renderer.texture.SpriteContents,java.util.List<SpriteOutline.Point>> PIXELS=new WeakHashMap<>();
    private static java.util.List<SpriteOutline.Point> opaque(net.minecraft.client.renderer.texture.SpriteContents sprite) {
        return PIXELS.computeIfAbsent(sprite,image -> {
            int[] left=new int[image.height()],right=new int[image.height()];java.util.Arrays.fill(left,image.width());
            for(int frame:image.getUniqueFrames().toArray()) for(int y=0;y<image.height();y++) for(int x=0;x<image.width();x++) if(!image.isTransparent(frame,x,y)) {
                left[y]=Math.min(left[y],x);right[y]=Math.max(right[y],x+1);
            }
            return SpriteOutline.fromRows(image.width(),image.height(),left,right);
        });
    }
        ...
        VertexConsumer consumer() {
            return new VertexConsumer() {
                ...
                @Override public void putBulkData(PoseStack.Pose pose,net.minecraft.client.renderer.block.model.BakedQuad quad,float[] brightness,float red,float green,float blue,float alpha,int[] light,int overlay,boolean readAlpha) {
                    // Use the visible silhouette, not its UV bounding rectangle: a diagonal sprite
                    // becomes a narrow upright weapon after rotation, with empty corners excluded.
                    ...
                    var outline=SpriteOutline.clip(pixels,(lowU-sprite.getU0())/spanU,(lowV-sprite.getV0())/spanV,(highU-sprite.getU0())/spanU,(highV-sprite.getV0())/spanV);
                    for(var pixel:outline) {
                        float uu=sprite.getU(pixel.u())-u[0],vv=sprite.getV(pixel.v())-v[0];
    ...
    void apply(FixtureBlockEntity fixture,int slot,ItemStack stack,BakedModel model,ItemRenderer renderer,PoseStack pose,boolean shelf) {
        ...
        if(layout==null||layout.model!=model||layout.revision!=fixture.revision()||layout.transform!=transform) {
            ...
            var fit=CompartmentFit.of(new AABB(bounds.minX,bounds.minY,bounds.minZ,bounds.maxX,bounds.maxY,bounds.maxZ),shelf,slot,fixture.slotCount());
